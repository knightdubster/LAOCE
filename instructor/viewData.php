...
if(isset($p_eval)){
	$evalID=strtoupper($p_eval);
	}else{
	$coursename='none';//sets variable to prevent sql error
	}
...
//ADMIN OR ID ACCESS?
		...
		//access granted
        	...
        	//CHECK COURSEINFO TABLE FOR INSTRUCTOR	
			$query = mysql_query("SELECT instructorID from CourseInfo WHERE evalID='$evalID'");
			$result = mysql_fetch_array($query);
			if($userID==$result['instructorID']){
				//access granted
			}else{//access denied
...
<head>
<title>Instructor Data View</title>
...
<div id="content" class='roundcorners'>
		...
		<div>
			...
			Instructor: <?php echo $instructor; ?><br/>
			<?php echo $submitted; ?> Submissions
			
			</div>
		</div><br/>
<?php
//set array for use by display code
//code from here on is the same as displayScores.php
$evalIDs[$i] = $evalID;
